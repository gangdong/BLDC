 ...
 * @author Dong Gang
 */
public abstract class GraphicViewer extends JPanel implements MouseMotionListener {

    /** This is the width and height of this panel.
     * 
     */
    protected int width_x = 900, height_y = 540;
     ...
     */
    public GraphicViewer(JScrollBar scrollBar, int mag) {
        /* 加载滚动条 */
        this.scrollBar = scrollBar;
        this.mag = mag;
        /* 设定属性 */
        //this.setSize(width_x*2, height_y*2);
        //this.setp
        proMag();
        this.setPreferredSize(new Dimension(width_x, height_y));
    ...
    public void proMag() {
        if (mag == 1) {
            ...
            height_y = 540 * 5;
            y_scale_length = 500 * 5;
            z0 = new Point(80, 510 * 5);
        }
    }
    ...
    void proMagDataBaseArray() {
        if (mag == 1) {
        } else if (mag == 2) {
            for (int i = 0; i < dataBaseArray.size(); i++) {
                Point p = (Point) dataBaseArray.get(i);
                Point pwr = new Point();
                pwr.x = p.x;
                pwr.y = p.y * 5;
                dataBaseArray.set(i, pwr);
            }
        }
    }
    ...
    void displayGraphicData(int size) {
        ...
        Point srcGridPoint = new Point();
        Point desGridPoint = new Point();
        ...
        for (int i = 1; i < size; i++) {
            ...
            if (srcPoint.y == z0.y + 20) {
                ...
                srcGridPoint = new Point(srcPoint.x,srcPoint.y - 20);
                desGridPoint = new Point(srcPoint.x,srcPoint.y-y_scale_length-20);
                Line2D.Float axis = new Line2D.Float(srcGridPoint, desGridPoint);
                g2d.draw(axis);
                /* 恢复默认笔刷 */
                g2d.setStroke(new BasicStroke(2, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_ROUND));
